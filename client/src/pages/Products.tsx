...
export default function Products() {
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (countryFilter === "unassigned")
            result = result.filter(
                (p) => !p.countryIds || p.countryIds.length === 0,
            );
        else if (countryFilter !== "all")
            result = result.filter(
                (p) => p.countryIds && p.countryIds.includes(countryFilter),
            );
    ...
    const onSubmit = (data: z.infer<typeof productSchema>) => {
        if (editingId) {
            updateProduct(editingId, {
                ...
                image: editCreativeUrl || undefined,
            });
            toast({ title: "Product updated" });
        } else {
            const assignCountryIds =
    ...
    const [
        selectedCountryIdsForAssignment,
        setSelectedCountryIdsForAssignment,
    ...
    const commitCountryAssignment = () => {
        ...
        toast({
            title: `Assigned ${selectedIds.size} products to ${newCountryIds.length} countries`,
        });
                ...
                if (!hasNameHeader || type === "paste") {
                    Papa.parse(content, {
                        ...
                        complete: (res: Papa.ParseResult<any>) => {
                            ...
                            if (
                                rows.length > 0 &&
                                /name|product|sku/i.test(rows[0].join(""))
                            ) {
                                startIdx = 1;
                            }
                            const mapped = rows
                                ...
                                .map((row: string[]) => {
                                    const sku = row[0] || "";
                                    const name =
                                        row[1] || row[0] || "Unknown Product";
                                    if (row.length === 1)
                                        return { sku: "", name: row[0] };
                                    return { sku, name };
                                })
                                .filter((p) => p.name);
                    ...
                    const mapped = items
                        .map((row: any) => {
                            ...
                            const nameKey = keys.find((k: string) =>
                                /name|product/i.test(k),
                            );
                            const skuKey = keys.find((k: string) =>
                                /sku|code|id/i.test(k),
                            );
                            return {
    ...
    const commitImport = () => {
        const assignCountryIds =
            countryFilter !== "all" && countryFilter !== "unassigned"
                ...
                : [];
        const countryName =
            assignCountryIds.length > 0
                ? countries.find((c) => c.id === assignCountryIds[0])?.name
                : null;
        ...
        setImportPreview([]);
        setPasteContent("");
        const msg = countryName
            ? `Imported ${newProducts.length} products → ${countryName}`
            : `Imported ${newProducts.length} products`;
        toast({ title: msg });
    };
    ...
    const totalCreatives = filteredProducts.reduce((sum, p) => {
        const count =
            p.creatives && p.creatives.length > 0
                ? p.creatives.length
                : p.image
                  ? 1
                  : 0;
    ...
    return (
        <div className="space-y-6">
                        ...
                        >
                            ...
                            <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Copy/paste from Excel or upload a CSV.
                                        Defaults: Draft, Cost 0, Price 0.
                                        {countryFilter !== "all" &&
                                            countryFilter !== "unassigned" && (
                                                <span className="block mt-1 font-medium text-primary">
                                                    Products will be assigned
                                                    to:{" "}
                                                    {
                                                        countries.find(
                                                            (c) =>
                                    ...
                                    >
                                        <div className="space-y-2">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <label
                                                    className={`flex-1 flex flex-col items-center justify-center h-20 border-2 border-dashed rounded-lg cursor-pointer hover:border-primary/50 hover:bg-muted/50 transition-colors ${editCreativeUploading ? "animate-pulse pointer-events-none" : ""}`}
                                                >
                                                    <div className="flex items-center gap-2 text-muted-foreground">
                                                        <ImagePlus className="w-5 h-5" />
                                                        <span className="text-sm">
                                                            {editCreativeUploading
                                                                ? "Uploading..."
                                                                : editCreativeUrl
                                                                  ? "Change"
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            delete the selected {idsToDelete.length} product
            ...
            <Dialog
                open={isAssignCountryOpen}
                onOpenChange={setIsAssignCountryOpen}
            >
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Assign Countries</DialogTitle>
                        <DialogDescription>
                            Select countries to assign to the selected{" "}
                        ...
                        <div className="space-y-2">
                            {countries.map((c) => (
                                ...
                                >
                                    <Checkbox
                                        id={`country-${c.id}`}
                                        checked={selectedCountryIdsForAssignment.has(
                                            c.id,
                                        )}
                    ...
                    <DialogFooter>
                        <Button onClick={commitCountryAssignment}>
                            Assign
                        </Button>
                    </DialogFooter>
                </DialogContent>
                            ...
                            filteredProducts.map((product) => (
                                <TableRow key={product.id} className="group">
                                    ...
                                    <TableCell className="font-medium">
                                        ...
                                        {product.countryIds &&
                                            product.countryIds.length > 0 && (
                                                <div className="flex gap-1 mt-1 flex-wrap">
                                                    {product.countryIds.map(
                                                        (cid) => {
                                                            const c =
                                                                countries.find(
                                                                    (co) =>
