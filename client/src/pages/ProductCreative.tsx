...
import { ArrowLeft, Upload, Download, Trash2, ImagePlus, Play, Video, Plus, X, FileArchive, CheckSquare, Square } from "lucide-react";
...
const isMediaFile = (name: string) => {
  ...
  return ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'mp4', 'webm', 'mov', 'avi', 'mkv', 'ogg'].includes(ext);
};
...
export default function ProductCreative() {
  ...
  const handleImportZip = async (file: File) => {
    ...
    try {
      ...
      if (mediaFiles.length === 0) {
        toast({ title: "No images or videos found in ZIP", variant: "destructive" });
        return;
      }
      ...
      const updated = [...creatives, ...newPaths];
      await updateProduct(product.id, { creatives: updated, image: updated[0] || undefined });
      toast({ title: `${newPaths.length} creative(s) imported from ZIP` });
    } catch (err) {
  ...
  const handleExportSelected = async () => {
    if (selectedIndices.size === 0) {
      toast({ title: "Select creatives to export", variant: "destructive" });
      return;
    }
    setExporting(true);
    try {
  ...
  const previewCreative = previewIndex !== null ? creatives[previewIndex] : null;
  const isPreviewVideo = previewCreative ? isVideoFile(previewCreative) : false;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-muted-foreground truncate">{product.name} {product.sku ? `(${product.sku})` : ''}</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          {selectMode && selectedIndices.size > 0 && (
            ...
            >
              ...
              {exporting ? 'Exporting...' : `Export (${selectedIndices.size})`}
            </Button>
          )}

          <label className={importingZip ? 'pointer-events-none' : ''}>
      ...
      {selectMode && creatives.length > 0 && (
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg bg-muted/50 border">
          ...
          <span className="text-sm text-muted-foreground">
            {selectedIndices.size === 0 ? 'Select all' : `${selectedIndices.size} of ${creatives.length} selected`}
          </span>
        </div>
      )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Badge variant="secondary" className="gap-1">
                {isPreviewVideo ? <><Video className="w-3 h-3" /> Video</> : <><ImagePlus className="w-3 h-3" /> Image</>}
              </Badge>
              <span className="text-sm text-muted-foreground">Creative {previewIndex! + 1} of {creatives.length}</span>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="gap-1" onClick={() => handleDownload(previewCreative, previewIndex!)} data-testid="button-preview-download">
                <Download className="w-4 h-4" /> Download
              </Button>
              <Button variant="destructive" size="sm" className="gap-1" onClick={() => handleRemove(previewIndex!)} data-testid="button-preview-remove">
                <Trash2 className="w-4 h-4" /> Remove
              </Button>
              <Button variant="ghost" size="sm" onClick={() => setPreviewIndex(null)} data-testid="button-preview-close">
                <X className="w-4 h-4" />
              </Button>
            </div>
          </div>
      ...
      {creatives.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[300px] border-2 border-dashed rounded-xl bg-muted/20 text-muted-foreground">
          ...
          <p className="text-xs mt-1 opacity-70">Use "Add Creatives" or "Import ZIP" to get started</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {creatives.map((path, index) => {
            const isVideo = isVideoFile(path);
            const isPreviewing = previewIndex === index && !selectMode;
            ...
            return (
              <div
                ...
                className={`group relative rounded-lg overflow-hidden border-2 cursor-pointer transition-all hover:shadow-md ${
                  isPreviewing ? 'border-primary ring-2 ring-primary/20' :
                  isChecked ? 'border-blue-500 ring-2 ring-blue-500/20' :
                ...
                onClick={() => {
                  if (selectMode) {
                    ...
                    setPreviewIndex(isPreviewing ? null : index);
                  }
                }}
              ...
              >
                ...
                {selectMode && (
                  <div className="absolute top-1.5 left-1.5 z-10">
                    <div className={`w-6 h-6 rounded border-2 flex items-center justify-center transition-colors ${isChecked ? 'bg-blue-500 border-blue-500' : 'bg-white/90 border-gray-400'}`}>
                      {isChecked && <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>}
                    </div>
                  </div>
                )}
